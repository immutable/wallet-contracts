...
abstract contract ModuleManager is Storage, EIP712, IModuleManager, RegistryAdapter {
  ...
  function _installValidator(
    address validator,
    bytes calldata data
  ) internal virtual withHook withRegistry(validator, MODULE_TYPE_VALIDATOR) {
  ...
  function _uninstallValidator(address validator, bytes calldata data) internal virtual {
    ...
    validator.excessivelySafeCall(
      gasleft(),
      0,
      abi.encodeWithSelector(IModule.onUninstall.selector, disableModuleData)
    );
  }
  ...
  function _installExecutor(
    address executor,
    bytes calldata data
  ) internal virtual withHook withRegistry(executor, MODULE_TYPE_EXECUTOR) {
  ...
  function _installHook(
    address hook,
    bytes calldata data
  ) internal virtual withHook withRegistry(hook, MODULE_TYPE_HOOK) {
  ...
  ) internal virtual withHook withRegistry(handler, MODULE_TYPE_FALLBACK) {
