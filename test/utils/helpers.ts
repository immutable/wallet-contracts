// Copied from https://github.com/0xsequence/wallet-contracts/blob/master/tests/utils/helpers.ts

import { ethers, BytesLike, BigNumberish } from "ethers";
import { MainModule } from "../../typechain-types";
...
export const createTestWallet = (web3: any, addressIndex: number = 0) => {
  ...
  const wallet = ethers.Wallet.fromMnemonic(
    process.env.npm_package_config_mnemonic!,
...
export function RevertError(errorMessage?: string) {
  if (!errorMessage) {
    ...
    return new RegExp(
      `VM Exception while processing transaction: reverted with reason string ["']${errorMessage}["']`
    );
  }
}
...
export async function ethSign(
  ...
  return ethsigNoType.endsWith("03") || ethsigNoType.endsWith("02")
    ? ethsigNoType
    : ethsigNoType + "02";
}
      ...
      if (typeof a.owner === "string" && !a.signature) {
        ...
        const signature = ethers.utils.arrayify(
          a.signature
            ...
            : await ethSign(a.owner as ethers.Wallet, message, hashed)
        );
        if (forceDynamicSize || signature.length !== 66) {
          const address =
            typeof a.owner === "string" ? a.owner : a.owner.address;
          return ethers.utils.solidityPack(
...
export async function multiSignAndEncodeMetaTxn(
  ...
  return wallet.interface.encodeFunctionData("execute", [
    txs,
    nonce,
    signature,
  ]);
...
export function encodeNonce(space: BigNumberish, nonce: BigNumberish) {
  const shiftedSpace = ethers.BigNumber.from(space).mul(
    ethers.constants.Two.pow(96)
  );
