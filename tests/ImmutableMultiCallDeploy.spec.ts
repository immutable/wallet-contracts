...
describe('Wallet Factory', function () {
  ...
  describe('deployAndExecute', function () {
    it('Should deploy and execute transfers', async function () {
      ...
      const { factory, mainModule, multiCall, acc1, owner, networkId, optimalGasLimit, token } = await loadFixture(
        setupFactoryFixture
      )
    ...
    it('Should fail with a CFA mismatch', async function () {
      ...
      const { factory, mainModule, multiCall, acc1, owner, networkId, optimalGasLimit } = await loadFixture(setupFactoryFixture)
  ...
  describe('deployExecute', function () {
    it('Should deploy and and execute transfer', async function () {
      ...
      // Execution
      await multiCall.connect(acc1).deployExecute(mainModule.address, salt, factory.address, [transaction], 0, sig)
      expect(await token.balanceOf(cfa)).to.equal(ethers.utils.parseEther('2.5'))
    })
    ...
    it('Should fail if the wallet is already deployed', async function () {
      ...
      await expect(
        multiCall.connect(acc1).deployExecute(mainModule.address, salt, factory.address, [transaction], 0, sigTwo)
