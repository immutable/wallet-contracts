...
export class Web3DebugProvider extends ethers.providers.JsonRpcProvider {
  ...
  constructor(web3Provider: ethers.providers.ExternalProvider, network?: ethers.providers.Networkish) {
    ...
    if (!web3Provider || !this._sendAsync) {
      ethers.logger.throwError('invalid web3Provider', ethers.errors.INVALID_ARGUMENT, {
        arg: 'web3Provider',
        value: web3Provider
      })
    }
...
export function encodeMessageData(owner: string, message: string, networkId: BigNumberish): string {
  return encodeMessageSubDigest(owner, ethers.utils.keccak256(message), networkId)
}

export function encodeMessageSubDigest(owner: string, digest: string, networkId: BigNumberish): string {
  return ethers.utils.solidityPack(['string', 'uint256', 'address', 'bytes32'], ['\x19\x01', networkId, owner, digest])
}
...
export async function walletSign(owner: ethers.Wallet, message: string, forceDynamicSize: boolean = false) {
  ...
  const accountBytes = await Promise.all(
    sorted.map(async a => {
      if (typeof a.owner === 'string' && !a.signature) {
        return ethers.utils.solidityPack(['uint8', 'uint8', 'address'], [1, a.weight, a.owner])
      } else {
        const signature = ethers.utils.arrayify(
          a.signature ? (a.signature as string) : await ethSign(a.owner as ethers.Wallet, message, hashed)
        )
        if (forceDynamicSize || signature.length !== 66) {
          ...
          return ethers.utils.solidityPack(['uint8', 'uint8', 'bytes'], [0, a.weight, signature])
        }
      }
    })
  )

  return ethers.utils.solidityPack(['uint16', ...Array(accounts.length).fill('bytes')], [threshold, ...accountBytes])
}
...
export async function signAndEncodeMetaTxn(
  ...
  return multiSignAndEncodeMetaTxn(wallet, [{ weight: 1, owner }], 1, txs, networkId, nonce, forceDynamicSize)
}
...
export async function signAndExecuteMetaTx(
  ...
  return multiSignAndExecuteMetaTx(wallet, [{ weight: 1, owner: owner }], 1, txs, networkId, nonce, forceDynamicSize)
}
...
export function moduleStorageKey(key: string, subkey?: string): string {
  ...
  return ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(['bytes32', 'bytes32'], [moduleStorageKey(key), subkey]))
}
...
export const WALLET_CODE =
  ''

export function addressOf(factory: string, mainModule: string, imageHash: string): string {
  const codeHash = ethers.utils.keccak256(
    ethers.utils.solidityPack(['bytes', 'bytes32'], [WALLET_CODE, ethers.utils.hexZeroPad(mainModule, 32)])
  )

  const hash = ethers.utils.keccak256(
    ethers.utils.solidityPack(['bytes1', 'address', 'bytes32', 'bytes32'], ['0xff', factory, imageHash, codeHash])
  )
